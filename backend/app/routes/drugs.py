...
async def seed_dataset_manual(db=Depends(get_database)):
    """
    Manually seed the drug composition dataset
    ...
    try: 
        ...
        await db.drug_composition_dataset.delete_many({})
        logger.info("Cleared existing drug composition dataset")
        ...
        result = await db.drug_composition_dataset.insert_many(default_data)
        logger.info(f"Manually seeded {len(result.inserted_ids)} drugs into dataset")
        
        return {
            "success": True,
            "message": f"Successfully seeded {len(result.inserted_ids)} drugs",
            "drugs": [drug["drugName"] for drug in default_data]
        }
        
    except HTTPException:
        raise
    except Exception as e:
        logger.error(f"Failed to manually seed dataset: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to seed dataset: {str(e)}"
        )
